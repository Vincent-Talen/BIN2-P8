...
# Create plot with raw values to demonstrate the need of medians
...
all.doses <- all.doses[!all.doses %in% (0)]
...
for (current.dose in all.doses) {
  ...
  margins <- theme(plot.margin = margin(0.5, 0.5, 0.25, 0.25, "cm"))
  
  # Create plot for receptor mRNA
  plot.Rm <- ggplot(cur.data.simulated, mapping = aes(x = time, y = Rm, width = 2)) + 
    ggtitle("Receptor mRNA") +
    geom_line() + ylab("Rm (fmol/g liver)") + xlab("time (h)") +
    geom_line(data = cur.data.medians, aes(color = "Red", width = 1), show.legend = F) +
    geom_point(data = cur.data, shape = 1) + margins
  
  # Create plot for free receptor concentration
  plot.R <- ggplot(cur.data.simulated, mapping = aes(x = time, y = R, width = 2)) +
    ggtitle("Free receptor concentration") +
    geom_line() + ylab("R (fmol/mg protein)") + xlab("time (h)") +
    geom_line(data = cur.data.medians, aes(color = "Red", width = 1), show.legend = F) +
    geom_point(data = cur.data, shape = 1) + margins
    

  grid.arrange(plot.Rm, plot.R, nrow = 1,
