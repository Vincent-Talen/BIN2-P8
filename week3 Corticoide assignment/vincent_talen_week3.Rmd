...
packages <- c("deSolve", "ggplot2", "gridExtra", "scales", "ggpubr")
...
```{r question 1, warning = FALSE, fig.align = "center", fig.height = 2.75}
...
```{r question 2, warning = FALSE, fig.align = "center", fig.height = 2.75}
...
<!-- (Question 3) -->
## Effect of different k.on and k.re values on receptor and mRNA dynamics
```{r question 3, warning = FALSE, fig.align = "center", fig.height = 2.75}
...
getDataSimulations <- function (new.values.string) {
  ...
  ## Run the simulation
  cur.scen.out <- ode(func = modelBasic, times = basic.times, y = basic.zero.state, parms = cur.scen.parameters)
  cur.scen.simulation <- as.data.frame(cur.scen.out)
  return(cur.scen.simulation)
}
```

```{r idk, warning = FALSE, fig.align = "center"}
mainFunction <- function(new.values) {
  ...
  plots <- lapply(c("Rm", "R", "DR", "DR_N"), a2.createPlot, simulations.names, simulations.data)
...
# Needs state variable info and creates plot with all simulations given in args
a2.createPlot <- function(state.var, simulations.names, simulations.data) {
  ...
  ## Create plot shell for current state variable
  plotje <- ggplot(basic.simulation, aes(x = time, y = !!sym(cur.var[["name"]]))) +
    labs(title = cur.var["title"], x = "time (h)", y = sprintf("%s (%s)", cur.var["name"], cur.var["unit"]))  +
    theme(legend.position = "bottom", plot.margin = margin(0.25, 0.25, 0.25, 0.25, "cm")) +
    xlim(NA, 48) +
    ## Add lines of all the simulations
    geom_line(aes(color = "Basic Simulation")) +
    unlist(mapply(function(cur.sim.name, cur.sim.data)
        geom_line(aes(color = cur.sim.name), data = cur.sim.data, linetype = "dashed"), simulations.names, simulations.data)) +
    ## Use scale_color_manual to color the data and also set their names in the legend
    scale_color_manual("", values = c("black", extra.colors), limits = c("Basic Simulation", simulations.names)) +
    ## Correct the displayed line types in the legend
    guides(color = guide_legend(override.aes = list(linetype = c(1, rep(2, length(simulations.names))))))
  return(plotje)
}
...
```{r question 4, warning = FALSE, fig.align = "center", fig.height = 2.75}

```

<!-- (Question 5) -->
## System dynamics with increased or decreased baseline mRNA production
```{r question 5, warning = FALSE, fig.align = "center", fig.height = 2.75}
