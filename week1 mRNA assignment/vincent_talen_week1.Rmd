...
\begin{center}
  \includegraphics[width=90mm]{biological_model.png}
\end{center}
...
model <- function(time, cur.state, parameters) {
  # Unpack the cur.transcripts.amt (R) and the parameters (r & m)
...
<!-- (Assignment 2) -->
# Assignment 2
```{r creating a plot showing 3 situations}
# Starting value and time frame
start.state <- c(R = 100)
times <- seq(0, 15, by = 0.01)

# Increasing R
incr.params <- c(r = 0.5, m = 60)
incr.out <- ode(y = start.state, times = times, func = model, 
                parms = incr.params, method = "euler")

# Steady state
steady.params <- c(r = 0.5, m = 50)
steady.out <- ode(y = start.state, times = times, func = model, 
                  parms = steady.params, method = "euler")

# Decreasing R
decr.params <- c(r = 0.5, m = 40)
decr.out <- ode(y = start.state, times = times, func = model, 
